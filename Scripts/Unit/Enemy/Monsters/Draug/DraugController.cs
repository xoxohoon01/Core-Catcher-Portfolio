...
public class DraugController : MonsterController
{
    ...
    public IEnumerator AttackInitialize()
    {
        ...
        obj1.Initialize(status.damage, 0, 0.2f, 0f, 1, 0, Faction.Monster, new Vector3(4, 4, 4));
        ...
        obj2.Initialize(status.damage, 0, 0.2f, 0f, 1, 0, Faction.Monster, new Vector3(4, 4, 4));
        ObjectPoolManager.Instance.Spawn("AudioObject", obj1.transform.position, Quaternion.identity).GetComponent<AudioObject>().PlayAudio("DraugAttack", "MonsterWeapon");
        yield return new WaitForSeconds(0.15f);
        ...
        obj3.Initialize(status.damage, 0, 0.2f, 0f, 1, 0, Faction.Monster, new Vector3(4, 4, 4));
        ObjectPoolManager.Instance.Spawn("AudioObject", obj1.transform.position, Quaternion.identity).GetComponent<AudioObject>().PlayAudio("DraugAttack", "MonsterWeapon");
    }
}
